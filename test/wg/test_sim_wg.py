...
def hllc_flux(UL, UR):
    ...
    if 0 <= SL:
        ...
        U_star = np.array([rhoL_star, rhoL_star * u_star, E_star])
        return FL + SL * (U_star - UL)
    elif S_star <= 0 <= SR:
        rhoR_star = rhoR * (SR - uR) / (SR - S_star)
        u_star = S_star
        E_star = (UR[2] / rhoR + (S_star - uR) * (S_star + pR / (rhoR * (SR - uR)))) * rhoR_star
        U_star = np.array([rhoR_star, rhoR_star * u_star, E_star])
        return FR + SR * (U_star - UR)
    else:
        return FR

def solve_euler_with_gravity(nx, x_domain, tf, CFL=0.9, g=-1.0):
    ...
    while t < tf:
        ...
        for i in range(1, nx-1):
            U[:, i] -= dt / dx * (F[:, i+1] - F[:, i])
        # Source term: add gravity
        for i in range(1, nx-1):
            U[1, i] += rho[i] * g * dt           # Momentum
            U[2, i] += rho[i] * u[i] * g * dt    # Energy
        # Boundary conditions
        U[:, 0] = U[:, 1]
        U[:, -1] = U[:, -2]
        t += dt
...
nx = 200
x, (rho, u, p) = solve_euler_with_gravity(nx, (0, 1), tf=0.2, g=-1.0)

# Plot
plt.figure(figsize=(12, 4))
plt.subplot(1, 3, 1)
plt.plot(x, rho)
plt.title("Density with Gravity")

plt.subplot(1, 3, 2)
plt.plot(x, u)
plt.title("Velocity with Gravity")

plt.subplot(1, 3, 3)
plt.plot(x, p)
plt.title("Pressure with Gravity")
